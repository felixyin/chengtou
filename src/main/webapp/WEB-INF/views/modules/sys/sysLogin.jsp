...
<form id="loginForm" class="form-signin" action="${ctx}/login" method="post">
    ...
    <%-- TODO 禁用短信验证码--%>
    <%--<label class="input-label ${username == 'admin' ? '':'my-sms-code'}" for="smsCode">短信验证码</label>--%>
    <%--<div class="input-append input-block-level ${username == 'admin' ? '' : 'my-sms-code'}">--%>
        <%--<input type="text" id="smsCode" name="smsCode" class="">--%>
        <%--<button id="smsCodeBtn" class="btn btn-default" type="button">获取</button>--%>
    <%--</div>--%>
...
<script type="text/javascript">
    $(function () {
        ...
        $('#smsCodeBtn').click(function () {
            // 发送短信验证码
            $.post('${ctx}/sendSmsCode', function (res) {
                if (res && res.isOk) {
                    alert('验证码发送成功');
                } else {
                    alert('验证码发送失败')
