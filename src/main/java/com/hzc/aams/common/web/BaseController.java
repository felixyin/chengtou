 ...
 */
package com.hzc.aams.common.web;
...
import com.hzc.aams.common.beanvalidator.BeanValidators;
...
//			}
    ...
    protected ServletContext getServletContent() {
        WebApplicationContext webApplicationContext = ContextLoader.getCurrentWebApplicationContext();
        ServletContext servletContext = webApplicationContext.getServletContext();
        return servletContext;
    }

    public void setSessionStatus(AamsProject aamsProject, HttpServletRequest request) {
        HttpSession session = request.getSession();
        if (StringUtils.isBlank(aamsProject.getStatus())) {
            Object statusObj = session.getAttribute("my-status");
            String status = String.valueOf(statusObj);
            if (statusObj != null && StringUtils.isNotBlank(status)) {
                aamsProject.setStatus(status);
            } else {
                aamsProject.setStatus(DictUtils.getDictValue("进行中", "project_types", "3"));
            }
        }
        session.setAttribute("my-status", aamsProject.getStatus());
    }
}
