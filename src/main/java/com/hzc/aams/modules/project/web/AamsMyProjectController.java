 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/project/my")
public class AamsMyProjectController extends BaseController {
    ...
    @RequiresPermissions({"project:my:view"})
...
//        搜索表单，设置默认下拉框选中
        ...
        return "modules/project/my/list";
    }
     ...
     * @return
     */
    public boolean isCurrentUser(AamsProject project) {
        boolean isCurrentUser = false;
        String sessionUserId = UserUtils.getUser().getId();
        if (StringUtils.isNotBlank(project.getId())) { // 修改数据的情况
            for (AamsProjectUser user : project.getAamsProjectUserList()) {
...
//       添加督办项目界面，设置默认下拉框选中
        ...
        if (StringUtils.isBlank(aamsProject.getStatus())) {
            aamsProject.setStatus(DictUtils.getDictValue("待审批", "project_types", "2"));
        }
...
//        }
        model.addAttribute("aamsProject", aamsProject);
        model.addAttribute("isCurrentUser", isCurrentUser(aamsProject));
        model.addAttribute("isForEdit", true);
        return "modules/project/my/form";
    }
    ...
    public String view(AamsProject aamsProject, Model model) {
        ...
        return "modules/project/my/view";
    }

    @RequiresPermissions(value = {"project:my:save", "project:my:submit"}, logical = Logical.OR)
    ...
    public String save(AamsProject aamsProject, Model model, RedirectAttributes redirectAttributes) {
        ...
        return "redirect:" + Global.getAdminPath() + "/project/my/?repage";
    }

    @RequiresPermissions("project:my:delete")
    ...
    @RequiresPermissions("project:my:order")
